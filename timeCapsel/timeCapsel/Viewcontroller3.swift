...
class ViewController3: UIViewController {
    ...
    @IBAction func sousin(_ sender: AnyObject) {
        ...
        let defaultAction: UIAlertAction = UIAlertAction(title: "OK", style: UIAlertActionStyle.default, handler:{
            ...
            self.userDefaults.set(component.year, forKey: "year6")
            self.userDefaults.set(component.month, forKey: "month6")
            self.userDefaults.set(component.day, forKey: "day6")
            self.userDefaults.set(self.ttext.text, forKey: "Text6")
            self.userDefaults.synchronize()
            var Y=Int(self.userDefaults.string(forKey: "year6")!)
            var M=Int(self.userDefaults.string(forKey: "month6")!)
            let D=Int(self.userDefaults.string(forKey: "day6")!)
            self.ttext.text=""
            self.label2.text=NSLocalizedString("半年後にメッセージを送りました。", comment: "")
            if M<7{
                M=M!+6
            }else{
                Y=Y!+1
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        label2.text=NSLocalizedString("半年後に届けるメッセージです", comment: "")
        
        if let tt=userDefaults.string(forKey: "Text6"){
            ttext.isHidden=true
            sousint.isHidden=true
            var Y=Int(userDefaults.string(forKey: "year6")!)
            var M=Int(userDefaults.string(forKey: "month6")!)
            let D=Int(userDefaults.string(forKey: "day6")!)
            //var A=0
            if M<7{
                M=M!+6
            }else{
